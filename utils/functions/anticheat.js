...
async function anticheat_update(client) {
  ...
  users.forEach(async user => {
    ...
    let pourcent =
      ([
        ...messages.map(
          x => x.have_spawn_card / 2 + (x.userCard == x.user_id ? 3 + 1 / 2 : 0)
        ),
        ...spawn.map(x => 1),
      ]?.reduce((a, b) => a + b, 0) /
        messages.length) *
    ...
    client
      ...
      .update({
        anticheat: pourcent,
        can_spawn: pourcent < 65 ? 1 : pourcent >= 75 ? 0 : undefined,
      })
      .where({ id: user.id })
      .catch(err => console.error(err));
  });
  ...
  guilds.forEach(async guild => {
    ...
    if (messages.length <= 7) return; // Pas assez de messages pour calculer le pourcentage
    let pourcent =
      ([...messages.map(x => x.have_spawn_card)]?.reduce((a, b) => a + b, 0) /
        messages.length) *
      100;
    pourcent = pourcent > 100 ? 100 : pourcent < 0 ? 0 : pourcent;
    if (!pourcent) return;
