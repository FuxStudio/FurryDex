...
module.exports = {
  ...
  async run(client, interaction) {
    ...
    if (serverConfig.last_Card == null) {
      let message =
        locales.already[serverConfig.locale] ?? locales.already["en-US"];
      interaction.reply(
        message.replace("%@player%", `<@${interaction.user.id}>`)
      );
      return;
    }
    ...
    if (card.possible_name.includes(guess)) {
      ...
      client
        ...
        .catch(err => console.error(err));
      let message =
        locales.congrat[serverConfig.locale] ?? locales.congrat["en-US"];
      interaction.reply(
        message
          ...
          .replace(
            "%cardId%",
            `${ID}, ${live < 0 ? live : `+${live}`}%/${attacks < 0 ? attacks : `+${attacks}`}%`
          )
