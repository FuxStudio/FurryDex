...
["commands", "buttons", "selects", "modals"].forEach(
...
if (!debug) {
  ...
  process.on("uncaughtException", (err, origin) => {
    ...
    Logger.error(
      client,
      `${"uncaughtException".toUpperCase()}: ${err}\nOrigin: ${String(origin)}`
    );
  });
  process.on("unhandledRejection", (reason, promise) => {
    logger.log(
      "error",
      `Unhandled Rejection at: ${Promise.resolve(promise)}\nReason: ${reason}`
    );
    Logger.error(
      client,
      `${"unhandledRejection".toUpperCase()}: at ${Promise.resolve(promise)}\nReason: ${reason}`
    );
  });
  ...
  client.rest.on("invalidRequestWarning", invalidRequestWarningData => {
    logger.log(
      "warn",
      `Invalid request warning: ${JSON.stringify(invalidRequestWarningData)}`
    );
    Logger.warn(
      client,
      `${"invalidRequestWarning".toUpperCase()}: ${invalidRequestWarningData}`
    );
  });
  client.on("warn", info => {
  ...
  client.on("shardDisconnect", (event, id) => {
    logger.log(
      "error",
      `Shard disconnected: ID ${id}, Event: ${JSON.stringify(event)}`
    );
    Logger.shard(
      client,
      `${"shardDisconnect".toUpperCase()} - ID: ${id}: ${event}`
    );
  });
  client.on("shardError", (event, id) => {
    logger.log(
      "error",
      `Shard error: ID ${id}, Event: ${JSON.stringify(event)}`
    );
  ...
  client.on("shardReady", (event, id) => {
    logger.log(
      "info",
      `Shard ready: ID ${id}, Event: ${JSON.stringify(event)}`
    );
  ...
  client.on("shardResume", (id, event) => {
    logger.log(
      "info",
      `Shard resumed: ID ${id}, Event: ${JSON.stringify(event)}`
    );
    Logger.shard(
      client,
      `${"shardResume".toUpperCase()} - ID: ${id}: ${event}`
    );
  });
}
...
client.on("messageCreate", message => {
  ...
  client.logger.log(
    "info",
    `/ Message received... Processing card spawning process...`
  );
  if (message.channel.isDMBased())
    return client.logger.log(
      "info",
      `\ DM channel detected. Ignoring message.`
    );
  if (client.config.bot.disable.bot)
    if (message.guild.members.cache.hasAny(...client.config.bot.disable.bot))
      return client.logger.log(
        "info",
        `\ Blacklisted bot in the guild. Ignoring message.`
      );
  if (message.author.bot)
    return client.logger.log(
      "info",
      `\ Message from a bot detected. Ignoring message.`
    );
  client.logger.log(
    "info",
    `| Message passed firsts checks. Proceeding to check if card spawning is allowed...`
  );

  isXMinutesPassed(message, client).then(result => {
    require("./utils/functions/anticheat.js").anticheat_message(
      client,
      message,
      message.author.id,
      result ? 1 : 0
